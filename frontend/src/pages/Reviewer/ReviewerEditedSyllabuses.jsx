...
import { fetchReviewerEditedSyllabuses } from "../../api/api";
...
export default function ReviewerEditedSyllabuses() {
  ...
  useEffect(() => {
    ...
    fetchReviewerEditedSyllabuses({ reviewerId: user.id, departmentId: deptId })
      .then((data) => {
        ...
        // Filter out APPROVED, REJECTED, and DRAFT - only show PENDING_REVIEW
        const filtered = arr.filter(item => item.status === "PENDING_REVIEW");
        setItems(filtered);
      })
      .catch((error) => {
        console.error("Error fetching edited syllabuses:", error);
        setItems([]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-slate-600 max-w-3xl">
          Review updated versions of syllabuses that were previously approved or rejected. Compare the new version with the previous one using AI, then approve or reject the changes.
        </p>
      </div>
        ...
        <div className="grid gap-3">
          {filtered.map((item) => {
            ...
            <button
              key={item.id}
              type="button"
              onClick={() => navigate(`/reviewer/edited-syllabuses/${item.id}`, { state: { readOnly: isReadOnly } })}
            ...
            >
              <div className="space-y-1">
                ...
                <div className="text-xs text-slate-500">
                  Last edited: {item.updated_at ? (item.updated_at.includes('T') ? item.updated_at.slice(0, 16).replace('T', ' ') : item.updated_at.slice(0, 16)) : ""}
                </div>
              </div>
          ...
          {!loading && filtered.length === 0 && items.length === 0 && (
            <div className="text-sm text-slate-500 border border-dashed border-slate-200 rounded-xl p-6 text-center">
              <div>No edited syllabuses found.</div>
