// ReviewerEditedSyllabusDetail.jsx - For edited syllabuses
...
export default function ReviewerEditedSyllabusDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([syllabusData, deps]) => {
        ...
        console.log("[REVIEWER EDITED] Syllabus data received:", {
          syllabusData,
          course: syllabusData?.course,
          department_id: syllabusData?.department_id,
          department_name: syllabusData?.department_name,
          department_code: syllabusData?.department_code,
          course_department: syllabusData?.course?.department,
        });
        console.log("[REVIEWER EDITED] Departments list:", deps);
        ...
        console.log("[REVIEWER EDITED] Extracted deptId:", deptId);
        ...
        console.log("[REVIEWER EDITED] Found department from list:", dept);
        ...
        if (syllabusData?.department_name || syllabusData?.department_code) {
          ...
          console.log("[REVIEWER EDITED] Setting department label from serializer:", label);
          setDepartmentLabel(label);
        } else if (dept) {
          const label = `${dept.name}${dept.code ? ` (${dept.code})` : ""}`;
          console.log("[REVIEWER EDITED] Setting department label from dept list:", label);
          setDepartmentLabel(label);
          ...
          const label = deptName ? `${deptName}${deptCode ? ` (${deptCode})` : ""}` : "";
          console.log("[REVIEWER EDITED] Setting department label from course:", label);
          setDepartmentLabel(label);
        } else {
          console.warn("[REVIEWER EDITED] No department found in any source");
          setDepartmentLabel("");
        }
      })
      .catch((e) => {
  ...
  const handleCompareVersions = async () => {
    ...
    try {
      const results = await compareSyllabusVersions({ reviewerId: user.id, syllabusId });
      setComparisonResults(results);
      setShowComparison(true);
    } catch (e) {
      const errorMsg = e?.response?.data?.detail || e?.response?.data?.message || e?.message || "Failed to compare versions";
  ...
  const handleSubmitDecision = async () => {
    ...
    try {
      ...
      navigate("/reviewer/edited-syllabuses", {
        ...
        <div className="mx-auto w-full max-w-6xl px-4 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="inline-flex items-center gap-2 text-[11px] font-extrabold uppercase tracking-[0.2em] text-indigo-700">
              ...
              Review Updated Syllabus Version
            </div>
          </div>
              ...
              {!showDecisionForm && (
                <>
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-gradient-to-r from-violet-600 to-purple-500 text-white text-sm font-extrabold shadow-lg hover:shadow-xl transition disabled:opacity-60"
                  >
                    <DocumentMagnifyingGlassIcon className="h-5 w-5" />
                    {compareLoading ? "Comparing..." : "Use AI to Compare with Previous Version"}
                  </button>
                  <button
                    type="button"
          ...
          {comparisonResults && showComparison && (
            <div className="rounded-3xl border border-violet-200 bg-violet-50/80 backdrop-blur p-6 shadow-sm">
              <div className="flex items-center gap-2 font-extrabold text-violet-900 mb-3">
                <DocumentMagnifyingGlassIcon className="h-5 w-5" />
                Version Comparison Results
              </div>
              <div className="text-sm text-slate-600 mb-2">
                Changes between the previous version and this updated version:
              </div>
              <div className="text-sm text-slate-800 whitespace-pre-wrap bg-white/70 rounded-2xl p-4 border border-violet-200">
                {comparisonResults.differences || comparisonResults.summary || comparisonResults.changes || "Comparison completed. Differences identified."}
              </div>
            </div>
          )}
