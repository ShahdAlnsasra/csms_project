...
export default function DeptCourseDiagramPage() {
  ...
  const handleDownloadDiagram = async () => {
    ...
    try {
      const dataUrl = await toPng(diagramBoxRef.current, {
        cacheBust: true,
        pixelRatio: 2,
      ...
      a.href = dataUrl;
      a.download = `department_curriculum_${department?.code || "diagram"}.png`;
      document.body.appendChild(a);
      a.click();
      a.remove();
    } catch (e) {
      alert("Could not export the diagram image.");
    }
  };
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          <button
            ...
            className="group inline-flex items-center gap-2 px-4 py-2 rounded-2xl border border-slate-200 bg-white text-xs font-semibold text-slate-700 shadow-md hover:shadow-lg hover:border-indigo-200 hover:text-indigo-700 transition"
          >
            <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 group-hover:bg-indigo-100 transition">
              <ArrowLeft className="h-3.5 w-3.5" />
            </span>
            Back to dashboard
          </button>
          <button
            ...
            className="group inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-gradient-to-r from-indigo-600 to-violet-600 disabled:opacity-50 text-xs font-semibold text-white shadow-lg shadow-indigo-200 hover:shadow-xl hover:from-indigo-500 hover:to-violet-500 transition"
          >
