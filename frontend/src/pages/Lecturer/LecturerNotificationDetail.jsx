...
import {
  fetchLecturerNotificationDetail,
...
export default function LecturerNotificationDetail() {
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true);
        setError("");
        const data = await fetchLecturerNotificationDetail(notificationId);
        setN(data);
        await markLecturerNotificationRead(notificationId);
      } catch {
  ...
  const handleOpenSyllabusDetails = async () => {
    ...
    try {
      setOpenLoading(true);
      const items = await fetchLecturerSyllabuses({
        lecturerId: user.id,
        courseId: n.course,
      });
      const list = Array.isArray(items) ? items : [];
      if (!list.length) {
        alert("No syllabus found for this course.");
        return;
      }
      ...
      navigate(`/lecturer/courses/${n.course}/versions/${target.id}`);
    } catch {
