// frontend/src/components/SystemAdminLayout.jsx
import React, { useState, useEffect } from "react";
import { Outlet, useNavigate, NavLink } from "react-router-dom";
import { ArrowRightOnRectangleIcon } from "@heroicons/react/24/solid";

// 🔹 Navbar של System Admin – אפשר להשתמש בו גם בתוך Terms/Privacy/Support
export function SystemAdminNavbar() {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = () => {
      setAvatar(localStorage.getItem("csmsAdminAvatar"));
    };
    ...
    <header className="border-b border-slate-200 bg-white shadow-sm sticky top-0 z-20">
      ...
      <nav className="border-t border-slate-100 bg-white">
        <div className="max-w-6xl mx-auto flex gap-6 px-4">
          <NavLink to="/system-admin" end className={navClass}>
            Overview
          </NavLink>
          <NavLink to="/system-admin/requests" className={navClass}>
            Requests
          </NavLink>
          <NavLink to="/system-admin/departments" className={navClass}>
            Departments
          </NavLink>
        </div>
      </nav>
...
// 🔹 הלייאאוט של System Admin משתמש ב־SystemAdminNavbar
