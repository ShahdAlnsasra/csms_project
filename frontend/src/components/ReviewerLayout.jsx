...
export function ReviewerNavbar() {
  ...
  const fullName = `${firstName} ${lastName}`.trim() || "Reviewer";
  const initials =
    ((firstName?.[0] || "") + (lastName?.[0] || "")).toUpperCase() || "R";
    ...
    <header className="border-b border-slate-200 bg-white shadow-sm sticky top-0 z-20">
      ...
      <nav className="border-t border-slate-100 bg-white">
        <div className="max-w-6xl mx-auto flex gap-6 px-4">
          <NavLink to="/reviewer/dashboard" end className={navClass}>
            Home
          </NavLink>
          <NavLink to="/reviewer/new-syllabuses" className={navClass}>
            New
          </NavLink>
          <NavLink to="/reviewer/edited-syllabuses" className={navClass}>
            Edited
          </NavLink>
